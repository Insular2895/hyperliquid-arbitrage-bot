...
| 17 | Candidate-grid/refinement matches exhaustive small cases; deterministic constraints hold; missing distribution gates stay unresolved | Unsupported size returns zero/reject; slicing cannot enlarge q; no fake default-pass distribution | Provisional sizing mechanics report; authoritative support pending Phase 18 feedback |
| 18 | F0 history and F1 arrival/mechanics emit reproducible q-dependent distributions and the same Sizer re-evaluates all gates | Fidelity claims cannot exceed modeled/calibrated mechanisms; missing distribution rejects | Simulator F0/F1 report plus authoritative sizing re-evaluation |
