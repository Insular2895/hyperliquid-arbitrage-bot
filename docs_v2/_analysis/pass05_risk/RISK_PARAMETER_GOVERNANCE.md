...
| `CALIBRATED` | Infra degradation thresholds/windows/hysteresis | metric-specific | Infra/client / Risk+Infra | Ordered `HEALTHY`→`DEGRADED`→`UNSAFE`; critical alert severity maps to `UNSAFE` | Benchmark + shadow/micro-live | Atomic only | YES | Fault/soak and false-trigger review |
