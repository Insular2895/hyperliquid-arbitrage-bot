...
## PASS 07 — Inventory / Capital / Bridge / Sizing target overlay

The authoritative row-level routing for all **545** Inventory/Capital/Sizing-index requirements is `pass07_inventory_capital/INVENTORY_CAPITAL_REQUIREMENT_LEDGER.md`. Inventory/Capital/Bridge/Sizing/Portfolio/Accounting-interface material targets `08_INVENTORY_AND_CAPITAL.md` and `deep-specs/inventory-capital/01..11`. Market Graph, routes, direct-comparator ownership, Market Atlas and HOT/WARM/COLD route to PASS08; expressions/units route to Formula Index/PASS11; completed Risk, Execution, Simulator, Participants and Data domains retain their primary ownership. Destinationless requirements: **0**; silently moved primary ownership: **0**; stable IDs renumbered: **0**.

## PASS 08 — Graph / Routes / Atlas / Quant target overlay

The authoritative disposition rules for **574** Routing rows and **597** Quant rows are `pass08_graph_routes_quant/GRAPH_ROUTING_REQUIREMENT_LEDGER.md` and `pass08_graph_routes_quant/QUANT_REQUIREMENT_LEDGER.md`. The indexes share 158 IDs and preserve one stable identity per requirement. Graph/Route/OWA/Triangle/HWC/Atlas material targets `03_MARKET_GRAPH_AND_ROUTES.md` and `deep-specs/market-graph/01..09`; Quant/Microstructure material targets `05_MARKET_MICROSTRUCTURE.md` and `deep-specs/market-microstructure/01..07`. Formula expressions/units remain Formula Index/PASS11-owned; completed domains retain their masters; exchange facts route to the external register; deployment/security/product/cross-exchange items retain future-pass ownership. Destinationless requirements: **0**; silently moved primary ownership: **0**; stable IDs renumbered: **0**.

## PASS 09 — Deployment / Security / Client Distribution target overlay

The authoritative disposition rule for all **426** Deployment/Security-index requirements is `pass09_deployment_security/DEPLOYMENT_REQUIREMENT_LEDGER.md`. Deployment, OCI/runtime security, client isolation/distribution, licensing and their operations interfaces target `14_DEPLOYMENT_AND_DOCKER.md` and `deep-specs/deployment-security/01..12`. Completed Infrastructure, Risk, Execution and Data/Recorder domains retain their masters; Validation/Operations requirements retain future-pass ownership; exchange/runtime/provider facts route to the external register. Destinationless requirements: **0**; silently moved primary ownership: **0**; stable IDs renumbered: **0**.

## PASS 10 — Validation / Operations target overlay

The authoritative deterministic routing for all **1,048** Validation/Operations-index requirements is `pass10_validation_operations/VALIDATION_REQUIREMENT_LEDGER.md`. Validation/maturity/evidence/promotion material targets `16_VALIDATION_MATRIX.md` and `deep-specs/validation/01..12`; health/metrics/alerts/SLO/runbook/incident/review material targets `18_OPERATIONS_AND_MONITORING.md` and `deep-specs/operations/01..10`. Completed-domain contracts retain their primary masters; formula audit routes to PASS11; implementation journey routes to PASS12; current external facts route to PASS13/14/external revalidation; calibrated/open items remain registered. Destinationless requirements: **0**; silently moved primary ownership: **0**; stable IDs renumbered: **0**.

## PASS 11 — Formula Book target overlay

The authoritative mathematical routing for all **153** stable `REQ-FORMULA-*` requirements and **110** canonical QFs is `04_FORMULA_BOOK.md`, `deep-specs/formulas/01..12`, verified `FORMULA_INDEX.md`, and analysis under `pass11_formula_book/`. Deterministic examples/parity target `GOLDEN_VECTOR_REQUIREMENT_CATALOG.md`; current exchange facts target the external registers; source-omitted estimator/invalid-case choices target `OPEN_ITEMS_INITIAL.md`; earlier-domain discrepancies target future cross-domain review without rewriting their masters. Destinationless formula requirements: **0**; silently moved primary ownership: **0**; stable IDs renumbered: **0**.

## PASS 12 — Roadmap target overlay

The authoritative PASS 12 routing for **142** reviewed roadmap units is `17_IMPLEMENTATION_ROADMAP.md`, `19_BUILD_VALIDATE_SCALE_ROADMAP.md`, `deep-specs/roadmaps/01..10` and `pass12_build_validate_scale/`. Technical phase profiles target master 17 and the phase matrices; evidence-stage profiles target master 19 and evidence/capital/learning matrices. Stable `REQ-VALID-0273..0367` retain PASS10 ownership where they define maturity/validation and gain PASS12 roadmap destinations where they define sequencing/scientific progression. Formula definitions, Risk permission, Execution state, Data truth and current external facts retain their owners. Destinationless requirements: **0**; stable IDs renumbered: **0**; implementation started: **NO**; PASS13 started: **NO**.
