...
## 28. Future participant and regime research

Participant, response, regime and adverse-selection features may later enter the versioned Feature Registry and `StrategySpec` only with point-in-time availability, declared support, temporal OOS and lineage to the same canonical outcome evidence. A research portfolio may compare regime-scoped candidates, but it cannot invent a participant truth, multiply headline probabilities onto `Π_exec`, or switch Live strategies automatically.

Sparse/OOD regimes abstain or use a declared supported fallback. Search and Monte Carlo preserve dependence among participant response, latency, liquidity and execution outcomes. Drift in these inputs creates evidence and research/demotion review, not in-place model or Risk mutation. See [Feature Registry](deep-specs/strategy-research/02_FEATURE_REGISTRY.md) and [Drift](deep-specs/strategy-research/10_DRIFT_AND_RESEARCH_TRIGGERS.md).
