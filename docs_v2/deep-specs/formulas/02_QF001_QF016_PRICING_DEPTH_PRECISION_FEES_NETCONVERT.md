...
`NetConvert` preserves both physical settlement and economic comparison evidence. Its result must semantically carry the input/output deltas, every fee/rebate delta (including a third asset), residual/fill facts and versions. A B terminal quantity never erases a fee paid in A or C. When values in different assets must be aggregated, the consumer uses an explicit point-in-time valuation asset, valuation method/source, freshness, rounding and version; missing or stale conversion evidence fails closed. The source does not select a universal numeraire or universal mark-versus-executable convention.

Exact-once invariant: every real fee/rebate affects economic value once and actual wallet deltas once. A fee already incorporated by QF-016 is not subtracted again by Route output, ExecutionEV, RAEV, BridgeCost or Accounting; a side-asset fee is not ignored merely because it does not reduce B.
