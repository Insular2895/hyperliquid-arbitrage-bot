...
| QF-071 | `(BridgeCost+ExpectedExitCost)/E[PnL_cycle]` | cycles | numerator aligned; expected cycle PnL>0 | denominator≤0 → conceptual ∞/never break even; numerator≤0 operational convention is OPEN and typed unresolved | positive numerator; zero/negative numerator unresolved; non-positive denominator |
...
The source names `RiskCost(P)` and `RelocationRiskCost` but does not define a provably disjoint partition. Until that boundary is governed, a candidate that cannot prove exact-once ownership across path execution risk, destination-state risk, ExpectedExitCost, StrandedPenalty, InventoryPenalty, ExecutionEV and Recovery is invalid for Bridge permission. No plausible interpretation is promoted to LOCKED merely by convention.
