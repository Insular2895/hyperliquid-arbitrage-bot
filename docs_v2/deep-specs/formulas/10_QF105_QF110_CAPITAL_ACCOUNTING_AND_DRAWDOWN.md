...
| QF-108 | `StrategyPnL=ΣRoutePnL+ΣRecoveryPnL+ΣRebalancePnL+InventoryPnL`; `EconomicPnL=StrategyPnL-InfraCost` | numeraire | disjoint strategy attribution, same period; compact economic equality is bridge-free scoped | duplicate attribution or use as general global close when Bridge applies is invalid | component isolation, zero-Bridge scoped equality and QF-106 reconciliation |
...
CORR-06 closes the remaining source-scope ambiguity conservatively. QF-106 is the global close for every period. QF-108's `StrategyPnL` is a subtotal that excludes the separately owned Bridge/Relocation bucket; its compact second equality is used only when `BridgePnL=0` for the declared scope. Otherwise the QF-106 close is mandatory. This is a consumer/precondition clarification, not a formula edit.
