...
## 50. CORR-06 — Final closure and infrastructure laboratory

Current Hyperliquid evidence retrieved 2026-09-09 distinguishes gossip/read priority from order/write priority. Read priority changes peer data propagation and is owned by feed/infrastructure experiments. Write priority has IOC temporal/mempool behavior and ALO queue-position behavior with different charge bases; it changes the versioned `Π_exec(q,state)` hypothesis while its actual/scenario charge is included once. It cannot bypass Risk, Reservations, protection or actual-fill truth.

Recorder can support bounded simultaneous multi-VPS campaigns that create empirical, versioned `InfraProfile` evidence. A later Counterfactual Replay may combine a market Dataset with a fresh-enough profile under an explicit manifest, but never rewrites RAW or labels modeled arrival/fill as actual. Candidate rental can end after calibration; the selected winner is monitored and challengers are reactivated briefly when drift, material change or a justified periodic check requires it. Passive Replay never replaces Micro-live actual fill/completion evidence. See [CORR-06 closure](_analysis/corr06_final_consistency/CORR06_FINAL_REPORT.md). `HDC-077..079` await final human review.

## 51. Scientific iteration and deferred strategy research

The same Core may later host a research plane following `OBSERVE -> UNDERSTAND -> HYPOTHESIZE -> TEST -> MEASURE -> VALIDATE -> PROMOTE -> MONITOR -> DEMOTE/RESEARCH`. Observation, hypothesis, simulation, authorization, execution truth and accounting remain separate. `StrategySpec`, feature/search artifacts, experiment reports and AI interpretations own no Risk, q, capital, ESM, Recovery, Reconciliation or Live authority.

`Trading Core -> Evidence/Recorder/Replay -> Research/Experiment -> Strategy Validation -> Operations/Monitoring -> reviewed candidate back to Trading Core`. Every arrow carries stable evidence/version lineage. Only the reviewed promotion gate can return a candidate; the research side cannot mutate canonical economic truth.

This future plane is activation-forbidden until the complete end-to-end baseline, deterministic evidence/reporting, Shadow and reconciled Micro-live outcome gates exist. Monte Carlo drives the existing canonical Simulator; it is not a second economic engine. Champion/Challenger and drift are later controlled layers with one execution owner, explicit human promotion and no online Live self-learning. Hyperliquid or infrastructure changes create evidence/revalidation and fail safe; they do not mutate production automatically. The roadmap remains exactly 26 phases. See [deferred strategy-research specifications](deep-specs/strategy-research/README.md). `HDC-080..090` await final human review.

## 52. Async concurrency and ordered commit

The modular monolith uses six responsibility classes: `C0 ORDERED_CANONICAL_COMMIT`, `C1 INLINE_BOUNDED_HOT_PATH`, `C2 SNAPSHOT_PARALLEL_COMPUTE`, `C3 ASYNC_EXTERNAL_EFFECT`, `C4 BACKGROUND_RUNTIME` and `C5 OFFLINE_RESEARCH`. They are architectural classifications, not mandated Rust enums or thread counts. The governing rule is **parallel to calculate, ordered to decide, single-writer to commit**.

Only C0 owners mutate canonical Book, Account, Fill, Inventory, Reservation, Risk, Execution, Recovery, Reconciliation and Accounting state. C2 work consumes immutable versioned snapshots and returns bounded-deadline proposals; current C0 revalidation, deterministic policy, final Risk and atomic Reservation remain mandatory. Stale results and worker completion order cannot authorize a trade. All queues/task populations are bounded; no hot-path await is indefinite.

Initial V1 keeps `pair_to_routes`, BBO, FastL1, full-L2 QF-016, small q-grid, current Risk, Reservation, immutable plan and local signing bounded/inline. Route/q/model workers are measured challengers, not assumed improvements. External order/cancel/query/reconciliation work is asynchronous and returns through ordered events. Recorder disk/checkpoint serialization/compression/archive and telemetry formatting/export are background. Research stays offline. `BATCH_SELECT` and deterministic `EARLY_COMMIT` remain policy candidates requiring Replay + Shadow; first-worker-wins is forbidden.

This is specified in [Async Concurrency and Ordered Commit](deep-specs/architecture/14_ASYNC_CONCURRENCY_AND_ORDERED_COMMIT.md) and the [analysis workspace](_analysis/async_concurrency_architecture/BASELINE_AND_SCOPE.md). `HDC-091..094` await final human review; implementation remains unauthorized.
