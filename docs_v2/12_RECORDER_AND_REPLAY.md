...
## 32. CORR-06 — Recorder as infrastructure laboratory

A bounded initial campaign runs candidate VPS recorders simultaneously over the same Hyperliquid periods. Cross-recorder alignment uses authoritative event/block/order identity, then a deterministic semantic fingerprint, then only an explicitly bounded fallback; nearest timestamp alone never proves identity. Local stages use monotonic clocks. Cross-machine lead/lag requires synchronized wall clocks and recorded uncertainty; results within uncertainty are `INCONCLUSIVE`.

Each campaign emits immutable Dataset references and empirical `InfraProfile` artifacts containing coherent sample vectors/distributions, tails, gaps, duplicates, reconnects, clock quality, scheduler/CPU/memory/network and Recorder pressure. It does not rewrite market RAW timestamps. Later `COUNTERFACTUAL LATENCY` runs combine a Dataset and profile through a fully pinned manifest and label their results `COUNTERFACTUAL`. Same Dataset/profile/seed/config reproduces the run. If correlated stages were measured, resample coherent vectors rather than independent impossible draws.

Candidate VPS need not remain rented after sufficient calibration. The winner runs with light continuous monitoring; challengers are briefly reactivated for a new candidate, material exchange/network/host/build change, observed degradation, drift or another justified revalidation trigger. Profile freshness has no invented calendar constant. Passive Replay cannot establish actual IOC/partial/completion/priority/Recovery outcomes and never replaces Micro-live. Multi-VPS pressure that loses critical P0/P1 evidence invalidates the experiment and invokes existing fail-safe policy. See the [CORR-06 profile contract](_analysis/corr06_final_consistency/REUSABLE_INFRA_PROFILE_AND_REPLAY_CONTRACT.md).

## 33. Recorder/Replay support for scientific iteration

Recorder evidence is the entry point for the future scientific loop. Immutable Dataset/Run manifests, point-in-time fields, gaps, missingness, failures and actual/counterfactual provenance feed hypotheses and experiments without rewriting RAW. Deterministic Replay must reproduce the frozen baseline before parameter search or Monte Carlo evidence is admissible.

A future report generator may derive hashes, counts and metrics from canonical artifacts, but human/AI text cannot overwrite those facts. Failed and losing runs remain addressable. Replay precedes Monte Carlo, then Shadow, then separately authorized Micro-live; none can create actual labels. See [Scientific Iteration](deep-specs/strategy-research/01_SCIENTIFIC_ITERATION_LOOP.md) and [Reporting](deep-specs/strategy-research/09_REPORTING_AND_KNOWLEDGE_BASE.md).

The future Strategy Discovery dataset view must retain point-in-time features, `OpportunityEpisodeId`, regime labels with availability/version, zero/partial/full/Recovery/reject/missed/expired and resolved UNKNOWN outcomes, actual reconciled economics, `InfraProfile`/feed/model versions and censoring. Offline episode/regime enrichment never enters RAW or leaks its later boundary/label into the original decision time.

## 34. Async durability and scheduler-independent Replay

The Core performs only a minimal bounded Recorder handoff. File write/flush/fsync, large serialization, compression, checksum, rotation, checkpoint encoding/write, archive, retention and formatting run in background classes. C0 captures a coherent checkpoint cursor/snapshot briefly; C4 serializes and writes it. P0 journal/account/fill/execution evidence is never silently dropped: threatened integrity disables new risk through existing health policy while safety/Recovery/Reconciliation remain available.

Replay permutes worker completion, delays, timeouts and counts while proving that transparent scheduling preserves `DecisionTrace = F(OrderedEvents, ResolvedConfig, ModelArtifacts, FormulaVersion, Seed)`. `CT-001..015` cover stale/no-return workers, Recorder/export pressure, ACK/fill/cancel races, slow reconciliation, worker flood, panic, archive failure, market gaps and two-vCPU oversubscription. See [Concurrency Replay Determinism](_analysis/async_concurrency_architecture/CONCURRENCY_REPLAY_DETERMINISM_CONTRACT.md).
