...
The CORR-05 snapshot treated user-controlled numeric paid order priority as unverified. CORR-06 supersedes that current-fact statement after fresh official revalidation; the exact-once ownership rule and absence of guaranteed benefit remain unchanged. See [Infrastructure Capture Composition](_analysis/corr05_economic_integration/INFRA_CAPTURE_ECONOMIC_COMPOSITION.md).

## CORR-06 — Current priority facts and reusable VPS profiles

As of 2026-09-09, official Hyperliquid documentation exposes independent gossip/read and order/write priority mechanisms. Gossip/read uses recurring on-chain slot auctions that affect normal and split client-block data only at hops configured to honor ordering; cost is paid from spot balance and burned. The documented latency effect is empirical, not guaranteed. Order/write grouping is eligible only for all-IOC or all non-reduce-only ALO non-outcome orders. IOC charges filled notional and changes temporal/mempool preference; ALO charges resting notional at placement and changes queue position within the documented recent tail. Write charges use undelegated staking balance converted to HYPE and are burned. These mutable facts require revalidation before activation.

Read-auction cost belongs to feed/infrastructure experiment economics; write priority cost belongs once in the affected execution scenario. Benefit flows only through a changed, validated outcome distribution. There is no generic `PriorityFee`, no autonomous PriorityOptimizer and no Risk bypass.

Infrastructure selection uses a bounded simultaneous candidate campaign, versioned empirical profiles, Replay/Shadow comparison, robust economic/reliability ranking, winner production monitoring and occasional event-driven challenger refresh. Mean latency alone cannot win over tails, gaps, availability, Recorder health, security/operations or net value. Old profiles are evidence with `FRESH`, `AGING`, `REVALIDATION_REQUIRED`, `STALE` or `INVALID` freshness—not permanent truth. Continuous challenger rental is not required. See the [multi-VPS protocol](_analysis/corr06_final_consistency/MULTI_VPS_CALIBRATION_PROTOCOL.md).

## Async I/O, CPU isolation and small-host budgets

Persistent network waits use asynchronous I/O; CPU-heavy route/model/simulation work never runs unbounded on I/O executor threads. Canonical reducers and safety/account/fill processing outrank optional compute. Recorder disk, compression/checkpoint/archive and telemetry use bounded background budgets; Replay/training/search use a separate process/resource envelope.

On a two-vCPU-class initial VPS, excess workers can worsen tails through run-queue delay, context switches, cache eviction and background interference. Worker count, queues, deadlines, affinity and shared-pool topology remain calibrated. Promotion requires semantic parity plus end-to-end queue/scheduler/state-age/capture/economic evidence; a microbenchmark or nominal core count is insufficient. Remote service hot-path dependencies remain zero. See [CPU versus I/O](./_analysis/async_concurrency_architecture/CPU_BOUND_VS_IO_BOUND_CONTRACT.md).
