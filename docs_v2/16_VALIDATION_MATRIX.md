...
## 49. Future strategy-research validation gates

Before any advanced candidate leaves Research, validate: point-in-time feature availability and no leakage; discovery/validation/final temporal holdout separation; purging/embargo where outcomes overlap; frozen walk-forward rules; all searched hypotheses/parameters/windows/seeds; justified multiplicity controls; baseline and sensitivity/stability surfaces; support/OOD/missingness; failures/censoring; calibrated coherent Monte Carlo convergence and exact-once economics; deterministic report hashes; Shadow population parity; and later reconciled Micro-live actual evidence.

Additional negative fixtures prove that `StrategySpec` status grants no Risk/Live permission, AI cannot promote or overwrite machine facts, a peak-PnL isolated optimum fails, arbitrary Gaussian/independent sampling fails, stale external/infra evidence makes affected scope NON-READY, failover cannot resume before fencing and reconciliation, and drift can only alert/contract/demote/research through existing authority. QF equations changed: 0. Risk and Execution semantics changed: 0. See [strategy-research specifications](deep-specs/strategy-research/README.md).

## 50. Async and concurrency validation

Validate the C0–C5 boundary before performance: no worker/effect/background direct mutation; bounded queues/task population; complete job/result versions; stale/late/duplicate disposition; no indefinite hot-path await; current final Risk; atomic Reservation; actual-fill-driven later legs; effect responses through ordered events; P0 Recorder preservation; deterministic startup/shutdown.

`CT-001..015` permute result order, staleness, no-return, Recorder/export pressure, ACK/fill/cancel order, slow reconciliation, worker flood, panic, archive outage, feed gap, worker counts and two-vCPU oversubscription. Transparent candidates require exact state/result/reason/version/DecisionTrace parity. Full-L2 worker, parallel q-grid, Participant/F2/F3, worker-count and `BATCH_SELECT`/`EARLY_COMMIT` candidates additionally report EventToDecision/Send P50/P95/P99/P99.9, queue/compute/scheduler, context switches, CPU/allocations, stale/deadline/saturation, Recorder interference, state age and capture/economics where material. See the [experiment matrix](_analysis/async_concurrency_architecture/ASYNC_PERFORMANCE_EXPERIMENT_MATRIX.md).
