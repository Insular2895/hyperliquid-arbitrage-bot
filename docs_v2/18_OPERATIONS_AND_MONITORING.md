...
## 51. Research, report and drift operations

Operations keeps research claims and machine facts distinguishable and preserves failed/censored/negative experiments. Incident, postmortem and release reports link canonical event/build/evidence identities, never rewrite them. Hyperliquid/API/fee/precision/node or infrastructure changes open dated evidence and revalidation; uncertain compatibility makes the affected scope NON-READY rather than silently updating production.

Future drift signals are typed, calibrated, support-aware and hysteretic. They may alert, contract scope through existing authority, request safe demotion, create an incident or open research; they cannot retune a Live artifact, raise Risk/capital/q or promote a challenger. Failover retains one fenced execution owner, boots NON-READY and reconciles exchange/account truth before readiness. See [Drift and Research Triggers](deep-specs/strategy-research/10_DRIFT_AND_RESEARCH_TRIGGERS.md) and the [failover contract](_analysis/future_strategy_research_and_lifecycle/INFRA_MAINTENANCE_AND_FAILOVER_CONTRACT.md).

## 52. Async runtime observability and degradation

For each bounded worker family record enqueue, queue-wait, start, compute-end, handoff, current-version revalidation, commit or typed discard, plus deadline/cancellation/fallback. Operations exposes backlog/full/saturation, retained bytes, scheduler delay, context switches, CPU, allocations where valid, stale/late/duplicate rates, effect timeouts, Recorder priority health and EventToDecision/Send/state-age tails. Raw job/route/order IDs remain in traces, not metric labels.

Account/fill/cancel/Recovery/Reconciliation work outranks optional workers. Worker panic, queue full, model unavailable, exporter/archive failure and background stalls remain scoped and cannot silently mutate truth. P0 Recorder threat, feed/account gap or unresolved effect disables new risk through existing health policy. See the [Failure Matrix](_analysis/async_concurrency_architecture/FAILURE_AND_DEGRADATION_MATRIX.md) and [Startup/Shutdown Contract](_analysis/async_concurrency_architecture/STARTUP_SHUTDOWN_CONCURRENCY_CONTRACT.md).

## 53. Phase 12 — alert authority and evidence populations

Operations observes and routes; it is not a second Risk authority. For `UNKNOWN_ORDER`, `BOOK_STALE`, `CLOCK_UNHEALTHY`, `INFRA_UNSAFE`, `RECOVERY_FAILED`, `SECRET_COMPROMISE` and `DISK_CRITICAL`, the canonical domain owner changes state/permission first and emits evidence; alert transport then notifies and invokes the runbook. Missing or duplicate notifications cannot suppress or duplicate the economic action.

Always qualify `IncidentSeverity.P0..P3` and `RecorderPriority.P0..P3`; they share labels but neither determines the other. Evidence freshness/validity is a runtime input, not only a dashboard field. Missing monitoring is unknown/unhealthy for the affected claim. Micro-live reports retain every pre-registered attempt, including zero/partial fills, Recovery, experiment/Risk stops, `UNKNOWN`, data loss, invalid observations and right censoring. Censored samples remain in population/completeness denominators without fabricated terminal outcomes.
