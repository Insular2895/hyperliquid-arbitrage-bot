...
| [ ] | SI-022 | The exact `ValidatedQSet = {q : Gates(q)=TRUE}` may be non-monotonic and contain holes; QF-076 defines its boundary `Q_validated = sup(ValidatedQSet)`. `q <= Q_validated` is not evidence that q is valid: every selected q independently satisfies `Gates(q)` unless monotonicity is separately proved for the exact versioned scope. More account capital does not enlarge the set. | size support is empirical, versioned and may contain holes | Validation | Sizer, Risk | unsupported or interval-inferred exposure | per-q gates plus any exact-scope monotonicity proof | [Formula Book QF-076](../04_FORMULA_BOOK.md#25-sizing--q_validated--qf-075077); Validation |
