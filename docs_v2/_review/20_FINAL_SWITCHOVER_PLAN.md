...
This is a deterministic **future** procedure. PASS 16 does not execute it. The recommended policy retains `_analysis` and `_review` inside the promoted canonical documentation for auditability; the human must explicitly accept that retention.

1. Obtain explicit approval quoting the exact PASS 16 commit SHA.
...
5. Re-run PASS 16 inventory, Markdown link, required-file, unchecked-gate and allowed-path validations.
...
14. Verify every 17 Master, 181 deep spec, 22 physical review files, PASS00–16 report and required register is present.
