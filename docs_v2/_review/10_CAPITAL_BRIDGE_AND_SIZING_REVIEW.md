...
`Q_validated` is a supremum/boundary, not a contiguous-interval certificate: a smaller q may fail while a larger one passes. Every chosen q evaluates exact gates. Size evidence is scoped to material slicing assumptions; temporal/adaptive changes require revalidation, while same-time fragmentation cannot manufacture depth.

Physical-resource disjointness does not prove tail independence. Overlapping L2 claims share capacity once, and unknown joint risk is constrained by Risk rather than invented by Portfolio. A non-atomic Bridge cannot assume certain destination utility; completion composition remains OPEN and inherits Phase 06 exact-once boundaries. QF-071 is diagnostic, STAY mandatory, and a candidate cannot justify the asset-class promotion that would make itself eligible.
