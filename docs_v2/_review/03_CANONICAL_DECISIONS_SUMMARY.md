...
| CD-019 | Priority read/write scopes and costs remain typed and exact-once | LOCKED boundary; external facts remain freshness-gated | Infra + Simulator + Accounting | no generic priority multiplier/optimizer | No blanket decision; revalidate each consumer |
| CD-020 | InfraProfile is expiring evidence, not truth or a Live dependency | LOCKED evidence boundary; profile selection remains empirical | Data + Recorder + Infra + Validation | no permanent challenger rental or actual-label leakage | No; later promotion stays scope-gated |
| CD-021 | Advanced strategy research is deferred, uses the same Core/Simulator and has no automatic/AI promotion | LOCKED scope boundary; Research/Future remains inactive | Architecture + Data + Simulator + Validation | baseline first; OOS/Shadow/Micro-live evidence; 26 phases only | No; any activation needs separate authority |
| CD-022 | Async I/O and bounded snapshot compute feed one ordered single-writer commit; completion order never chooses a trade | LOCKED architecture boundary; routing choices remain evidence-selected | Architecture + Data + Execution + Validation | inline baseline first; all queues/tasks bounded; route policy Replay + Shadow | No; implementation choice cannot widen semantics |

`HDC-001..094` trace post-source requirements; they are not 94 pending policy decisions. Human review confirms or rejects this baseline as a whole in the [decision form](21_FINAL_HUMAN_DECISION_FORM.md). Individual overrides must name the affected authorities, invariants, tests and traceability.
