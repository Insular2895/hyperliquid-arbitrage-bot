...
## 32. CORR-06 — Priority and infrastructure-profile evidence

Linked, schema-versioned evidence separates `GOSSIP_READ`, `IOC_WRITE` and `ALO_WRITE` policy scopes. It records requested policy/rate/grouping, eligibility, source-document version, observed sequencing fields and actual priority charge without relabeling it as exchange fee or infrastructure cost. Absent evidence is `UNAVAILABLE`, never numeric zero.

An `InfraProfile` evidence artifact binds its identity, provider/region/datacenter-or-AZ, instance/CPU/kernel/container/network/feed profiles, calibration window/count, Dataset, clock quality, empirical coherent latency vectors and tails, scheduler/CPU/memory/network/reconnect/gap/duplicate/Recorder/stability evidence, uncertainty, scope, timestamp and freshness state. Detailed IDs remain in trace data, not metric labels. Every profile Replay manifest pins `DatasetId`, `InfraProfileId`, Replay/RunMode, Simulator fidelity, build/commit, config/formula/models, feed, seed, clock and counterfactual-policy versions. Outcome class is exactly one of `ACTUAL_MEASURED`, `PAIRED_ACTUAL`, `REPLAYED`, `COUNTERFACTUAL`, `SHADOW`, `MICROLIVE_ACTUAL`, `LIVE_ACTUAL`.

## 33. Research artifact and feature lineage

Research records link—not replace—canonical `DatasetId`, `RunId`, `IncidentId` and `EvidenceId`. Human-readable `DATASET-*`, `RUN-*` or `INC-*` forms must be reversible serializations. Documentary `HYP-*`, `EXP-*`, `BENCH-*`, `STRAT-*`, `ADR-*`, `POST-*` and `REL-*` identities are versioned and superseded rather than overwritten; `EXT-*` continues the existing external register and `HDC-*` keeps its distinct approval role.

Every decision-time feature records event and availability timing, source/schema versions, lookback, missingness, support and point-in-time lineage. Future reports separate immutable generator-populated manifest facts from human/AI interpretation. Failed, negative, aborted, invalid, censored and inconclusive runs remain first-class rows. See the [research knowledge base](research/README.md) and [Feature Registry](deep-specs/strategy-research/02_FEATURE_REGISTRY.md).

Future interfaces are schema families, not prematurely frozen structs: `FeatureMetadata`, `StrategySpec`, `HypothesisRecord`, `ExperimentRecord`, `MonteCarloManifest`, `ResearchTrigger`, `StrategyPromotionEvidence` and `StrategyDemotionEvidence`. Each binds stable identity/version, scope, provenance, dependencies, evidence/support, status and supersession. Promotion/demotion evidence records an input to the existing authority workflow; it is not a command, Risk permission or actual-outcome source.

## 34. Concurrency provenance and acceptance

A snapshot worker request/result family binds job/family identity, ordered-event cursor, generation, creation/deadline/start/completion monotonic timing, complete input fingerprint, all consumed state/artifact/config/formula versions, output identity, support/OOD and validity envelope. This is a schema-family requirement under existing versioning rules, not an ad hoc mutation of frozen structs.

Only the ordered owner can accept a proposal. It checks identity, deadline, generation and every material current version, then deterministically commits, revalidates or discards. Cancellation is cooperative; correctness never assumes it succeeded. Result completion order is diagnostic evidence, not economic order. Transparent scheduling changes preserve `DecisionTrace`; policy-changing route selection is explicit/versioned. See the [Stale Result Contract](_analysis/async_concurrency_architecture/STALE_RESULT_AND_VERSIONING_CONTRACT.md).
