...
## 34. CORR-03 — Resolved route-outcome profile and baseline

For `ForecastLabelVersion=ROUTE_OUTCOME_RESOLVED_V1`, the four fields are a terminal resolved-attempt partition: `p_full` predicts original route completion proved by actual-fill-compatible objectives with no Recovery; `p_partial` predicts non-completion with one or more strategy fills and no Recovery; `p_recovery` predicts non-completion that enters Recovery, irrespective of recovery success/PnL; `p_failure` is the residual resolved zero-fill/no-Recovery non-completion. `UNKNOWN`, unresolved and invalid evidence are not put into `p_failure`; coverage is reported separately. Other label versions are not a partition unless they independently prove exclusivity and exhaustion.

Initial TT/TTT binds this profile to a versioned canonical-terminal resolution policy, not an arbitrary fixed-millisecond horizon. A finite-time profile with unresolved probability mass cannot claim a four-way terminal partition unless that mass is explicitly represented by its label schema.
...
CORR-03 did not decide how the distribution affects QF-056/057/063, sizing or QF-076. CORR-05 section 36 now closes that composition without changing Simulator math.
...
The Simulator may estimate downstream effects of a feed/infra profile, but it cannot make a challenger canonical, create actual execution outcomes or authorize a speculative send. QF-085 and CORR-03 completion evidence remain distinct inputs whose economic ownership is closed by CORR-05 section 36; Simulator math is unchanged.

## 36. CORR-05 — One execution-outcome distribution

The Simulator owns one versioned `Π_exec(q,state)` distribution. Under `ROUTE_OUTCOME_RESOLVED_V1`, F/P/R/X are conditional on an execution attempt and the frozen q/state/label/scope: F completes the original route without Recovery; P is noncomplete with strategy fill and no Recovery; R is noncomplete with Recovery entry; X is the resolved zero-fill/no-Recovery residual. Unknown/unresolved/censored/invalid coverage remains outside the partition.

QF-048/QF-085 survival and empirical completion evidence calibrate or condition this distribution. They are not multiplied onto its QF-056/057 EV unless a separately validated factorization proves distinct conditional events and no overlap. QF-059 and tail measures derive from the same distribution. Each scenario cashflow includes fees, executable book walk and path/Recovery economics once. See [Execution PnL Distribution Contract](_analysis/corr05_economic_integration/EXECUTION_PNL_DISTRIBUTION_CONTRACT.md).

## 37. CORR-06 — Priority treatments and reusable infrastructure profiles

Priority is a typed treatment, not a scalar multiplier. IOC write priority may alter effective ordering/latency; ALO priority alters queue position within the documented recent-order window and is not IOC logic. Their charged amounts enter the affected scenario cashflow once. Gossip/read priority instead changes a feed/arrival profile and its auction cost belongs to feed/infrastructure economics. All three effects remain empirical and versioned inputs to the same `Π_exec(q,state)` distribution.

`COUNTERFACTUAL LATENCY` may apply a versioned `InfraProfile` or declared `+Δt` treatment to an immutable Dataset. The manifest pins Dataset, profile, modes/fidelity, build/config/formula/models/feed, seed, clock policy and counterfactual policy. Results are `COUNTERFACTUAL`, never actual arrival or fill truth. Coherent sampled vectors preserve measured correlations; unsupported fields remain `UNAVAILABLE`. Profile Replay cannot prove IOC fills, partials, Recovery or priority benefit and cannot override contradictory Micro-live evidence. See the [reusable profile contract](_analysis/corr06_final_consistency/REUSABLE_INFRA_PROFILE_AND_REPLAY_CONTRACT.md).

## 38. Future Monte Carlo research boundary

Future Monte Carlo is an ensemble runner over this Simulator and its single versioned `Π_exec(q,state)` distribution. Search chooses candidate configurations; deterministic Replay reconstructs evidence; the Simulator evaluates canonical scenarios; Monte Carlo characterizes a coherent distribution. No second fill, PnL, cost or accounting engine is created.

Sampling uses real calibrated, support-aware empirical/bootstrap distributions before stronger parametric assumptions. It preserves measured dependence, failures, censoring, tails and OOD behavior; arbitrary Gaussian perturbations and impossible independent stage combinations are forbidden. Every path keeps fees, book walk, priority, Recovery, inventory and external penalties exact-once. Results remain `MODELLED`/`COUNTERFACTUAL`, cannot establish actual fills and must precede Shadow and Micro-live. See [Monte Carlo Robustness](deep-specs/strategy-research/05_MONTE_CARLO_ROBUSTNESS.md).
